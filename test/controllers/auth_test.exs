defmodule Whiskydb.AuthTest do
  use Whiskydb.ConnCase
  alias Whiskydb.Auth

  setup %{conn: conn} do
    conn =
      conn
      |> bypass_through(Whiskydb.Router, :browser)
      |> get("/")
  ...
  test "login puts the user in the session", %{conn: conn} do
    login_conn =
      conn
      |> Auth.login(%Whiskydb.User{id: 123})
      |> send_resp(:ok, "")
  ...
  test "call with no soession sets current_user assign to nil", %{conn: conn} do
  ...
  test "authenticate_user continues when the current_user exists", %{conn: conn} do
    conn =
      conn
      |> assign(:current_user, %Whiskydb.User{})
